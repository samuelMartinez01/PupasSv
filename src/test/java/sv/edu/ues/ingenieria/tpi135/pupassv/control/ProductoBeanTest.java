...
public class ProductoBeanTest {
    ...
    @Test
    void countByIdTipoProducto_valoresValidos() {
        Integer id = 1;
        Integer countEsperado = 5;

        TypedQuery<Integer> typedQuery = mock(TypedQuery.class);  // Cambiar a TypedQuery
        when(em.createNamedQuery("Producto.countByIdTipoProducto", Integer.class)).thenReturn(typedQuery);
        when(typedQuery.setParameter("idTipoProducto", id)).thenReturn(typedQuery);
        when(typedQuery.getSingleResult()).thenReturn(countEsperado);

        Integer count = bean.countByIdTipoProducto(id, null, null);

        assertEquals(countEsperado, count);
    }


    @Test
    void countByIdTipoProducto_excepcion() {
        Integer id = 1;

        when(em.createNamedQuery("Producto.countByIdTipoProducto", Integer.class)).thenThrow(RuntimeException.class);

        Integer count = bean.countByIdTipoProducto(id, 0, 10);
    ...
    void deleteRelacion_valoresValidos() {
        ...
        // Simular que la consulta a la base de datos devuelve un resultado
        ...
        when(mockQuery.executeUpdate()).thenReturn(1);  // Simular que la relación se eliminó

        // Mock del delete heredado (de AbstractDataAccess)
    ...
    void setEntityManager_noHaceNada() {
        ...
        assertEquals(em, bean.getEm()); // sigue siendo el original
    }
}
