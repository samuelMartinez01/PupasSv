...
public class ComboBeanIT {
    ...
    public void testFindProductosByComboId() {
        System.out.println("findProductosByComboId");
        ComboBean cut = new ComboBean();
        EntityManager em = emf.createEntityManager();
        cut.em = em;

        Long idCombo = 1L; // Asegúrate que este ID exista en tu pupas_ddl.sql
    ...
    public void testConvertirAComboDTO() {
        System.out.println("convertirAComboDTO");
        ComboBean cut = new ComboBean();
        EntityManager em = emf.createEntityManager();
        cut.em = em;

        Combo combo = em.find(Combo.class, 1L); // Asegúrate que exista este combo en pupas_ddl.sql
